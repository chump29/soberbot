...
const invoke = async (interaction: ChatInputCommandInteraction): Promise<void> => {
  ...
  await DB.getDate(interaction.user.id, interaction.options.getString("name") as string).then(
    async (msg: string): Promise<void> => {
      await interaction.reply({
        content: msg.includes("❌") ? `-# > ${msg}` : msg,
        flags: msg.includes("❌") ? MessageFlags.Ephemeral : MessageFlags.SuppressNotifications
      })
    }
  )
}
